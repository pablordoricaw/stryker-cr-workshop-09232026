...
A ~4-hour, self-paced, hands-on workshop where you build the **same end-to-end data project** on **your team's domain dataset** — Finance (Orthopedics), Security (infrastructure), or ITSM (IT-Ops) — **entirely in the Databricks Workspace UI with no local setup required**. You'll work through guided starter notebooks with `# TODO`s, validated at each checkpoint with `workshop.check()`, and get contextual help from **Genie Code** as you go. Everyone finishes the whole thing.

The result: ingest documents + transactional data → **medallion** (bronze → silver → gold) → auto-generate **governance metadata** → a **semantic layer** (UC Metric Views) → a natural-language **Genie agent** → a working **data app**.
...
You'll work through **three domain streams** — Finance (Orthopedics), Security (infrastructure), or ITSM (IT-Ops) — each building the same medallion architecture:
