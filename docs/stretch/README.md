...
version your provided infrastructure (schema + UC Volume) and your data app as
source-controlled YAML, so the whole thing is reviewable, re-runnable, and
promotable between workspaces.

This is a **packaging** exercise, not a re-provisioning one: your workshop
environment already exists (you built it from `00_setup` onward). The bundle is
a second, declarative *description* of the provided infra + app — the same infra
the maintainer bundle at the repo root (`databricks.yml`) describes, but named
for **you**.

- **CREATE-CATALOG-free.** The bundle's `catalog` variable names your
  **existing** catalog; the bundle defines only the schema, volume, and app
  inside it — never a catalog resource.
- **Namespace-aware.** The `schema`, `app_name` (and, if you add it, the
  Lakebase project) default to your identity-derived names, so a teammate can
  deploy their own copy of the same bundle without a clash.
- **Deterministic names.** Targets deliberately do **not** set
  `mode: development`; dev-mode name-prefixing injects characters that are not
  legal Unity Catalog identifiers and would drift from what your notebooks
  created.

The metric views and Genie agent are **not** DAB resource types you package
here — they live in your schema and are already governed by Unity Catalog.
...
> `databricks bundle validate`; deploy only if you want to.

The gated solution ships a **complete, validated example bundle** you can copy
and adapt: [`solutions/finance/stretch/package_as_dab_bundle/`](../../solutions/finance/stretch/package_as_dab_bundle/).
