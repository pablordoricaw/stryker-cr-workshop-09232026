...
# MAGIC Postgres driver the `07_app` checkpoint uses to verify served rows), but the
...
# 01_bronze_txn (supplied via the widget). 07_app does NOT create a project, and its
...
# The synced table lands in YOUR existing catalog + schema, and no catalog is
...
# MAGIC **Reuse the Lakebase project you created for `01_bronze_txn`**, since 07_app does
...
# MAGIC lands straight into your **existing** catalog/schema, with **no catalog to
...
# your existing catalog, with NO Lakebase catalog. NOTE: `.wait()` returns
# once the resource is provisioned, which can still be SYNCED_TABLE_PROVISIONING
# and does NOT block until the initial snapshot finishes. So poll below until
...
# MAGIC
# MAGIC 1. **Create** from the app switcher (top-left grid) → **Databricks Apps** →
