...
# MAGIC (`w.genie.create_space` / `update_space` / `list_spaces`) is available, but the
...
text_instructions = [
    ...
    "- Users are IT operations analysts, so use governed ITSM Metric Views for aggregates.",
    "",
    "## DISAMBIGUATION",
...
# untouched, and we create our own instead, so we never overwrite someone else's.
owned = None
for s in _pages():
