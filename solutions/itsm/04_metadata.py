...
# MAGIC
# MAGIC - **`comment`** for table + column descriptions;
# MAGIC - **`pi`** for a `data_classification` tag on sensitive columns; and
# MAGIC - **`domain`** for a `domain` tag on each table.
# MAGIC
...
target_tables = ["gold_incidents", "gold_service_performance"]
table_names = ",".join(
...
display(
    spark.sql(
        ...
        WHERE schema_name = '{config.schema}'
          AND table_name IN ('gold_incidents', 'gold_service_performance')
...
# MAGIC %md
# MAGIC ## 🛟 No-PyPI fallback for documenting the tables without dbxmetagen
# MAGIC
...
# MAGIC
# MAGIC Comments always apply; `SET TAGS` is **best-effort**, and under a governed tag
