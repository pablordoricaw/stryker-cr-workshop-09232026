...
# MAGIC its own review/log tables into that **same resolved schema**.
# MAGIC
...
target_tables = ["gold_findings", "gold_cve_exposure"]
table_names = ",".join(
...
display(
    spark.sql(
        ...
        WHERE schema_name = '{config.schema}'
          AND table_name IN ('gold_findings', 'gold_cve_exposure')
...
FALLBACK_PII_COLUMNS = {
    ...
    # gold_cve_exposure is an aggregated CVE mart with no direct PII; the domain's
    # PI requirement is met by gold_findings above.
    "gold_cve_exposure": [],
}
FALLBACK_TABLE_DESCRIPTION = {
    "gold_findings": "Finding-grain security fact: one row per vulnerability-scan finding with asset, CVE, severity, risk, and remediation attributes.",
    "gold_cve_exposure": "CVE-grain security mart: aggregated exposure, risk, and remediation metrics per CVE across assets.",
}
...
# MAGIC table carries a business-domain tag. The Security gold tables are passed
# MAGIC through the domain-generic checkpoint's `tables` extra.
