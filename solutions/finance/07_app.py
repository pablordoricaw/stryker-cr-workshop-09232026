...
# MAGIC
# MAGIC Everything here stays in the Databricks workspace, so no terminal is needed.
...
# MAGIC 01_bronze_txn's CDF history table (they don't collide, since it is a different table in a
...
# MAGIC %md
# MAGIC **GAP 1 of 2, the Genie Conversation API connection** (`Backend.ask_genie`):
# MAGIC ```python
...
# MAGIC ```
# MAGIC
# MAGIC **GAP 2 of 2, the Lakebase read** (`Backend.fetch_serving_rows`):
# MAGIC ```python
...
# MAGIC
# MAGIC Do all of this in the workspace, with no terminal needed. **Deploying an app starts it
