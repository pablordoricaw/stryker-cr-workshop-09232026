...
# MAGIC 2. **classify** each into one of the five Finance classes with
...
display(
    spark.table(silver_class_table("vendor_invoice")).select(
        "filename", "invoice_number", "vendor_name", "invoice_date", "total_amount"
    )
)
